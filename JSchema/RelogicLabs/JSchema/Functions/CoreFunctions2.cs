...
public sealed partial class CoreFunctions
{
    public bool Enum(JString target, params JString[] items)
    {
        if(!items.Contains(target))
            return Fail(new JsonSchemaException(
                ...
                new ExpectedDetail(Caller, $"string in list {items.JoinWith(", ", "[", "]")}"),
    ...
    public bool Enum(JNumber target, params JNumber[] items)
    {
        if(!items.Contains(target))
            return Fail(new JsonSchemaException(
                ...
                new ExpectedDetail(Caller, $"number in list {items.JoinWith(", ", "[", "]")}"),
    ...
    public bool Nonempty(JString target)
    {
        ...
        if(_length <= 0) return Fail(new JsonSchemaException(
